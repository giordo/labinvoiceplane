...
class Mdl_Uploads extends Response_Model
{
    ...
    {
        $this->load->model('product/mdl_products');
        $product = $this->mdl_products->get_by_id($id);
        $query = $this->db->query("Select file_name_new,file_name_original from ip_uploads where url_key = '" . $product->product_key . "'");
        $names = array();
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                array_push($names, array(
    ...
    {
        unlink(getcwd() . '/uploads/customer_files/'. $filename);                            
    }
